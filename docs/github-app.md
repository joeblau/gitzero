...
- Repository permissions: `Contents: read`, `Pull requests: read`, `Checks: write`, `Variables: read`, `Actions: read`, `Environments: read`, `Metadata: read`, and `Administration: write`. The administrative permission is used only by the explicitly confirmed repository-onboarding endpoint; readiness requests ask GitHub for `administration: read`, and webhook processing never requests or uses administrative write access.
