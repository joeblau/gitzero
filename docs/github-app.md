...
- Repository permissions: `Actions: read`, `Artifact metadata: read`, `Attestations: read`, `Checks: write`, `Code quality: read`, `Contents: read`, `Deployments: read`, `Discussions: read`, `Environments: read`, `Issues: read`, `Packages: read`, `Pages: read`, `Pull requests: read`, `Code scanning alerts: read`, `Commit statuses: read`, `Dependabot alerts: read`, `Variables: read`, `Metadata: read`, and `Administration: write`. The read permissions let unchanged workflows request the matching read-only `GITHUB_TOKEN` scopes, including `permissions: read-all`; every issued token still contains only that job's exact declared subset. Administrative write is used only by the explicitly confirmed repository-onboarding endpoint. Readiness and private shared-source policy checks request only `administration: read`; webhook processing never requests or uses administrative write access.
