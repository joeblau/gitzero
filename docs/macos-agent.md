...
Workflow and job `permissions` are enforced per job. The no-declaration baseline is repository contents and pull-request read access; explicit read/write mappings, `read-all`, `{}`, and reusable-workflow downgrades receive their exact scope. Any non-baseline token is requested over the agent WebSocket only after assignment, masked immediately, cached only for the current run, and cleared when the run ends. The Worker reconstructs the signed webhook before granting writes and reduces every fork or Dependabot PR write to read. OIDC and `write-all` requests stop the workflow as unsupported instead of silently granting incomplete authority.
