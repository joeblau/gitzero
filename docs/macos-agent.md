...
Workflow and job `permissions` are enforced per job. The no-declaration baseline is repository contents and pull-request read access; explicit mappings, `read-all`, `{}`, and reusable-workflow downgrades receive their exact read-only scope. Any non-baseline token is requested over the agent WebSocket only after assignment, masked immediately, cached only for the current run, and cleared when the run ends. Write and OIDC permission requests stop the workflow as unsupported instead of silently granting or ignoring authority.
