...
Jobs also expose `job.workflow_ref`, `job.workflow_sha`, `job.workflow_repository`, and `job.workflow_file_path`. For a direct job these identify the root workflow at the authenticated pull-request execution snapshot. A job expanded from a local or remote reusable workflow instead identifies that defining workflow's repository, file, requested ref, and pinned commit, so the standard checkout pattern using `repository: ${{ job.workflow_repository }}` and `ref: ${{ job.workflow_sha }}` works without changing the reusable workflow.
