...
For an unprotected job environment, the agent reports start and terminal lifecycle events around the workflow job, including a validated HTTP(S) environment URL after step outputs resolve. It never receives the Worker's deployment-write credential. The control plane durably mirrors those events into GitHub Deployment statuses. Object-form `environment.deployment: false` keeps metadata validation and environment-scoped variables but suppresses those lifecycle events.
